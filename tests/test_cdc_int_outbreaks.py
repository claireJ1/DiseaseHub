...
sys.path.append('./code/cdc_int_scrape/sample.json')
fpath = './code/cdc_int_scrape/sample.json'
...
def test_attributes():
    ...
    data_attributes = ["cases", "country", "deaths", "source", "date_of_report"]

    with open(fpath) as f:
        data = json.load(f)

    for event in data["events"]:
        assert len(event["attribute"]) == 5
